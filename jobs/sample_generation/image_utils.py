...
def get_rand_string_image_emnist(rand_string: str, dataset_type: str):
    images_to_concat = []
    handwritten_dataset_base_path = Path(
        f"./data/handwritten_dataset_emnist/{dataset_type}"
    )
    ...
    for character in rand_string:
        ...
        character_image_path = character_image_path / random.choice(
            os.listdir(character_image_path)
        )
        ...
        if random.choice([True, False]):
            ...
            pad = np.full(
                (character_image.shape[0], pad_size, 4),
    ...
    images_to_concat_processed = preprocess_images_to_concat(
        images_to_concat, processing="scaling"
    )
...
def get_rand_string_image(rand_string):
    ...
    for character in rand_string:
        character_image_path = (
            labels[labels["label"] == character].sample(n=1)["image"].iloc[0]
        )

        character_image = cv2.imread(
            f"{handwritten_dataset_processed_base_path}/{character_image_path}",
...
def add_string_image_to_document(
    document_image: np.array,
    ...
    start_x, start_y, end_x, end_y = denormalise_box_coordinates(
        box.start_x_norm,
        box.start_y_norm,
        box.end_x_norm,
        box.end_y_norm,
    ...
    rand_string_image_scaled = cv2.resize(
        rand_string_image, (new_width, new_height), interpolation=cv2.INTER_AREA
    )
    ...
    document_with_strings = overlay_image(
        background=document_image,
        overlay=rand_string_image_scaled,
        position=(new_start_x, new_start_y),
    )

    blank_document_with_strings = overlay_image(
        background=blank_document,
        overlay=rand_string_image_scaled,
        position=(new_start_x, new_start_y),
    )
    ...
    box_label = {
        "template_box_id": box.id,
        "name": box.name,
        "label": rand_string,
        "coords": [
            [int(start_x_buffer), int(start_y_buffer)],
            [int(end_x_buffer), int(start_y_buffer)],
            [int(end_x_buffer), int(end_y_buffer)],
            [int(start_x_buffer), int(end_y_buffer)],
        ],
    }
