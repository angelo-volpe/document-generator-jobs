...
from .image_utils import (
    get_rand_string_image_emnist,
    add_string_image_to_document,
    normalise_box_coordinates,
)
...
def get_annotations(sample_labels: list, filename: str):
    boxes_annotations = map(
        lambda x: {"transcription": x["label"], "points": x["coords"]}, sample_labels
    )
...
def run_sampling(
    ...
    output_base_path: Path = Path("./data/"),
    config_path: Path = Path("./jobs/sample_generation/config/default_config.yaml"),
):
    logger.info(
    ...
    document_image = cv2.imdecode(
        np.frombuffer(document_image_buffer, dtype=np.uint8), flags=cv2.IMREAD_UNCHANGED
    )

    # Generate samples
    labels = {}

    degradations_config = DegradationsConfig(config_path=config_path)
    degradations_list = degradations_config.get_degradations_list()
    for sample_id in tqdm(range(num_samples)):
        ...
        for box in boxes:
            ...
            logger.debug(
                f"random string parameters: length={string_length}, is_alphabetic={box.is_alphabetic}, is_numeric={box.is_numeric}"
            )
            rand_string = generate_random_string(
                string_length, box.is_alphabetic, box.is_numeric
            )
            ...
            if random.choice([True, False]):
                ...
                rand_string_image = apply_gaussian_grayscale(
                    rand_string_image, mean=int(grayscale_mean), std_dev=10
                )

            document_with_strings, blank_with_strings, box_label = (
                add_string_image_to_document(
                    document_image=document_with_strings,
        ...
        image_degradator = ImageDegradator(
            image=document_with_strings,
            boxes_labels=boxes_labels,
            degradations_list=random_degradations,
            degradations_config=degradations_config,
        )
        ...
        if publish:
            ...
            for box in boxes_labels:
                ...
                start_x_norm, start_y_norm, end_x_norm, end_y_norm = (
                    normalise_box_coordinates(
                        start_x, start_y, end_x, end_y, doc_width, doc_height
                    )
                )
            ...
            sample_document_id = publish_sample_image(
                image_path, sample_id, document_id
            )
