...
class ImageDegradation(ABC):
    ...
    def apply(self, image: np.array, boxes_labels: list = None):
        ...
        """
        logger.debug(
            f"Applying degradation {self.__class__.__name__} with parameters: {self.parameters}"
        )
        # save original image
        image = image.copy()
        if boxes_labels is not None:
            degradated_image, new_boxes_labels = self._apply_degradation(
                image, boxes_labels=boxes_labels, **self.parameters
            )
...
class WaveDistortionDegradation(ImageDegradation):
    ...
    def _apply_degradation(
        self, image: np.array, boxes_labels: list, amplitude: float, frequency: float
    ):
        ...
        for box in boxes_labels:
            ...
            new_start_x = np.where(np.ceil(X_distorted[start_y:end_y, :]) == start_x)[
                1
            ].min()
            new_end_x = np.where(np.ceil(X_distorted[start_y:end_y, :]) == end_x)[
                1
            ].max()

            new_box = box.copy()
            new_box["coords"] = [
                [int(new_start_x), int(start_y)],
                [int(new_end_x), int(start_y)],
                [int(new_end_x), int(end_y)],
                [int(new_start_x), int(end_y)],
            ]

            new_boxes_labels.append(new_box)

        distorted_image = cv2.remap(
            image, X_distorted, Y_distorted, interpolation=cv2.INTER_LINEAR
        )
    ...
    def _apply_degradation(
        ...
        if direction == "left":
            ...
            offset_x = random.randint(
                int(-width * len_percentage), int(width * len_percentage)
            )
            offset_y = random.randint(
                0, int(height * len_percentage)
            )  # Shadows usually fall down
...
class ColorFilterDegradation(ImageDegradation):
    ...
    def _apply_degradation(
        self, image: np.array, red: float = 1.0, green: float = 1.0, blue: float = 1.0
    ):
        """
...
class DegradationsConfig:
    def __init__(self, config_path: Path):
        ...
        for degradation, parameters in config.items():
            degradation_class = globals().get(degradation)
            params_range = {}
            for param_name, param_value in parameters.items():
                if isinstance(param_value, dict):
                    # If the value is a dictionary, create a range
                    params_range[param_name] = np.arange(
                        param_value["min"], param_value["max"], param_value["step"]
                    ).tolist()
                    ...
                    raise ValueError(
                        f"Invalid configuration for {degradation}: {param_value}"
                    )
            self.degradation_params_range[degradation_class] = params_range

    def get_params_range(self, degradation: ImageDegradation) -> Dict[str, any]:
        if degradation not in self.degradation_params_range:
            raise ValueError(f"Degradation {degradation} not found in config.")

        params_range = self.degradation_params_range[degradation]
        return params_range

    def get_degradations_list(self) -> List[ImageDegradation]:
        """
        Get the list of available degradations from the config.
        """
        return list(self.degradation_params_range.keys())
...
class ImageDegradator:
    def __init__(
        self,
        image: np.array,
        boxes_labels: List[dict],
        degradations_list: List[ImageDegradation],
        degradations_config: DegradationsConfig,
    ):
        self.image = image
    ...
    def apply_degradations(self):
        for degradation in self.degradations_list:
            ...
            logger.debug(
                f"Applying degradation: {degradation_instance.__class__.__name__}"
            )

            # Apply the degradation
            if isinstance(degradation_instance, WaveDistortionDegradation):
                self.image, self.boxes_labels = degradation_instance.apply(
                    image=self.image, boxes_labels=self.boxes_labels
                )
            else:
