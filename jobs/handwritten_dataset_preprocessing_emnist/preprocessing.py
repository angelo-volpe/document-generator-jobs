...
def add_alpha(image: np.array):
    ...
    rgba[:, :, 3] = (
        255 - image
    )  # A channel (inverse of grayscale - white becomes transparent)
...
def run_hw_preprocessing_emnist(dataset_type: str = "train"):
    ...
    logger.info(
        f"start preprocessing of {len(letters_imgs)} images for {dataset_type} dataset"
    )
    ...
    for image, label in tqdm(
        zip(letters_imgs, letters_labels), total=len(letters_imgs)
    ):
